...
for file in follower_files:
    data = np.genfromtxt(path_to_followers + file, skip_header = 1, dtype = int) #first row is header
...
all_followers = pd.DataFrame(all_followers, columns = ["user_id"], dtype = int)
followers_minus_tweeters = pd.DataFrame(followers_minus_tweeters, columns = ["user_id"], dtype = int)
...
for file in nofollower_files:
    data = np.genfromtxt(path_tweeters_nofollowers + file, dtype = int) #first row is header
    try:
...
for file in friend_files:
    data = np.genfromtxt(path_to_friends + file, skip_header = 1, dtype = int) #first row is header
...
for file in tweeters_nofriends_files:
    data = np.genfromtxt(path_to_tweeters_nofriends + file, dtype = int) #first row is header
    try:
        data = data[1:len(data)] #if empty, will return error
