########################################
#
# PLOT: Tweets over time, comparing articles from mainstream and fringe news sources
#
########################################
...
outpath <- 'output/timeseries/source_type/'
...
tweet_perhour <- tweets %>% 
  group_by(source_type, total_article_number, hour_bin) %>% 
  count(.)
...
t.test(relative_tweet_time~source_type, data = percentile_data)
...
gg_ideoldisttime <- tweets %>% 
  ...
  group_by(source_type, hour_bin, ideol_distance) %>% 
...
gg_ideoltime <- tweets %>% 
  ...
  group_by(source_type, article_lean, total_article_number, hour_bin) %>% 
  count(ideol_bin) %>% 
  mutate(freq_ideol_bin = n / sum(n)) %>% 
  group_by(source_type, article_lean, hour_bin, ideol_bin) %>% 
  ...
  theme(aspect.ratio = NULL, 
        legend.box.margin = unit(c(0, 0, 0, 0), "mm")) +
  facet_grid(source_type~article_lean,
             labeller = labeller(article_fc_rating = label_veracity))
gg_ideoltime
...
####################
# Load exposure data 
exposure_data <- read.csv('/Volumes/CKT-DATA/fake-news-diffusion/data_derived/timeseries/users_exposed_over_time.csv', header = TRUE, colClasses = c("user_id"="character")) %>% 
...
article_data <- tweets %>% 
  select(tweet_number, total_article_number, source_type, source_lean, article_fc_rating, article_lean, user_ideology, article_ideology) 
exposure_timeseries <- merge(exposure_data, article_data, by = c("tweet_number", "total_article_number")) %>% #NOTE: should merge on user ID
  group_by(total_article_number) %>% 
